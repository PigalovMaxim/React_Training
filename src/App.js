...
class App extends Component {
  ...
  componentDidMount() {
    if(localStorage.getItem('isUserLogin')){
      this.setState({isUserLogin: localStorage.getItem('isUserLogin'), access_token: localStorage.getItem('access_token')});
    }
  }
  render() {
    return (
        ...
        <Routes>
          {
            ...
            <Route path='/Game' element={<Game token={this.state.access_token} logout={this.logout}/>}/>
          }
        </Routes>
      </div>
    );
